...
func Register(db *gorm.DB, reg model.RegisterRequest) error {
	...
	// 验证用户名，密码和邮箱是否为空
	if reg.Username == "" {
		return pkg.NewBizError(pkg.CodeInvalidUsername)
	}
	if reg.Password == "" {
		return pkg.NewBizError(pkg.CodeInvalidPassword)
	}
	if reg.Email == "" {
		return pkg.NewBizError(pkg.CodeInvalidEmail)
	}
	...
	// 检查手机号非空的时候，数据库中是否已经存在了
	if reg.Phone != "" {
		db.Model(&model.User{}).Where("phone = ? ", reg.Phone).Count(&count)
		if count > 0 {
			return pkg.NewBizError(pkg.CodePhoneExists)
		}
	}
	// 对密码进行加密处理
	hashPassword, err := utils.GeneratePasswordHash(reg.Password, nil)
	if err != nil {
		return pkg.NewBizError(pkg.CodePasswordEncryptError)
	}
	// 创建用户
	user := model.User{
